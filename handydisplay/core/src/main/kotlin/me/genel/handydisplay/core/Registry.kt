...
@Suppress("UNCHECKED_CAST") inline fun <reified R: IRegisterable<R>> getAll(): Set<R>? {
    return registryEntries[R::class]?.toList()?.map { it.second }?.toSet() as Set<R>?
}


@Throws(AssertionError::class) fun assertRegistryNameValid(registryName: String) {
    assert(
...
class DuplicateRegistrationException(registryType: KClass<*>, key: String, entry1: Any, entry2: Any): Exception(
        StringBuilder().appendLine("Cannot register two items with the same registry name and registry type.")

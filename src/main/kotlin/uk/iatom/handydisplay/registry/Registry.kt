...
object Registry {
   ...
   */
  val registryEntries:
      MutableMap<KClass<out IRegistrable<*>>, MutableMap<String, IRegistrable<*>>> =
      LinkedHashMap()
  ...
  private inline fun <reified S : IRegistrable<S>> registerServices(clazz: KClass<S>) {
    ServiceLoader.load(clazz.java).iterator().forEach { register(it) }
  }
   ...
   * @param R The type (implementing `IRegistrable<R>`) of the sub-map.json into which the item
  ...
  private inline fun <reified R : IRegistrable<R>> register(item: R) {
    ...
    registryLogger.fine(
        "Registering ${item.registryName}=<${R::class.simpleName}>${item.javaClass.name}")
  ...
  inline fun <reified R : IRegistrable<R>> getAll(): Set<R>? {
    return registryEntries[R::class]?.toList()?.map { it.second }?.toSet() as Set<R>?
  }
}
...
fun getRegistryNameErrors(registryName: String): RegistryNameException? {
  ...
  if (!registryName.asSequence().all { it.isLetterOrDigit() }) {
    return RegistryNameException(
        registryName, "Internal widget names may " + "only contain letters and digits.")
  }

...
/**
 * Typically thrown by the `ModManager` while it is creating instances of mods' main classes, hence
 * the presence of a `Constructor<AbstractMod>?` in the constructor.
 */
...
private fun formatConstructorInfo(cons: Constructor<*>?): String {
  return if (cons != null)
      "${formatConstructorModifiers(cons.modifiers)} ${
...
private fun formatConstructorArgs(cons: Constructor<*>) =
    cons.parameterTypes.joinToString(", ", "(", ")") { it.name }
